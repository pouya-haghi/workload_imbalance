...
void sfc(void)
{
   ...
   MPI_Allreduce(&n_m_tmp, &n_m_tot, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    ...
    fprintf(outfile, "MPI_Allreduce-42: time: %lf \n", t_latency);
    fclose(outfile);
   t4 = timer();
   t2 = t4 - t1;
   if (n_m_tot) {  // Only move dots and blocks if there is something to move
    ...
    snprintf(my_str, sizeof(my_str), "/scratch/07431/tg867305/process_skew/miniamr/%d/p%d.txt", world_size, my_rank);
    outfile = fopen(my_str, "a");
    fprintf(outfile, "MPI_Alltoall-4: time: %lf \n", t_latency);
    fclose(outfile);
    ...
    snprintf(my_str, sizeof(my_str), "/scratch/07431/tg867305/process_skew/miniamr/%d/p%d.txt", world_size, my_rank);
    outfile = fopen(my_str, "a");
    fprintf(outfile, "MPI_Alltoall-5: time: %lf \n", t_latency);
    fclose(outfile);
    t_start = MPI_Wtime(); //added
    ...
    snprintf(my_str, sizeof(my_str), "/scratch/07431/tg867305/process_skew/miniamr/%d/p%d.txt", world_size, my_rank);
    outfile = fopen(my_str, "a");
    fprintf(outfile, "MPI_Allreduce-43: time: %lf \n", t_latency);
    fclose(outfile);
...
void sfc_sort(int div, int fact)
{
   ...
   MPI_Allreduce(&num_dots, &total_dots, 1, MPI_INT, MPI_SUM, comms[div]);
    ...
    snprintf(my_str, sizeof(my_str), "/scratch/07431/tg867305/process_skew/miniamr/%d/p%d.txt", world_size, my_rank);
    outfile = fopen(my_str, "a");
    fprintf(outfile, "MPI_Allreduce-44: time: %lf \n", t_latency);
    fclose(outfile);
   ...
   MPI_Allreduce(bin, gbin, max_bin, MPI_INT, MPI_SUM, comms[div]);
    ...
    snprintf(my_str, sizeof(my_str), "/scratch/07431/tg867305/process_skew/miniamr/%d/p%d.txt", world_size, my_rank);
    outfile = fopen(my_str, "a");
    fprintf(outfile, "MPI_Allreduce-45: time: %lf \n", t_latency);
    fclose(outfile);
   ...
   for (j = 0; j < (fact-1); j++)
    ...
    snprintf(my_str, sizeof(my_str), "/scratch/07431/tg867305/process_skew/miniamr/%d/p%d.txt", world_size, my_rank);
    outfile = fopen(my_str, "a");
    fprintf(outfile, "MPI_Allreduce-46: time: %lf \n", t_latency);
    fclose(outfile);
