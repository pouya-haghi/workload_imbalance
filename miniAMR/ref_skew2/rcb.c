...
void rcb(void)
{
   ...
   if (change_dir) {
    ...
    fprintf(outfile, "MPI_Allreduce-7: time: %lf \n", t_latency);
    fclose(outfile);
    t_start = MPI_Wtime(); //added
    ...
    snprintf(my_str, sizeof(my_str), "/scratch/07431/tg867305/process_skew/miniamr/%d/p%d.txt", world_size, my_rank);
    outfile = fopen(my_str, "a");
    fprintf(outfile, "MPI_Allreduce-8: time: %lf \n", t_latency);
    fclose(outfile);
         for (dir = 0; dir < 3; dir++)
   ...
   MPI_Allreduce(&n_m_tmp, &n_m_tot, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    ...
    snprintf(my_str, sizeof(my_str), "/scratch/07431/tg867305/process_skew/miniamr/%d/p%d.txt", world_size, my_rank);
    outfile = fopen(my_str, "a");
    fprintf(outfile, "MPI_Allreduce-9: time: %lf \n", t_latency);
    fclose(outfile);
   t4 = timer();
   t2 = t4 - t1;
   if (n_m_tot) {  // Only move dots and blocks if there is something to move
    ...
    snprintf(my_str, sizeof(my_str), "/scratch/07431/tg867305/process_skew/miniamr/%d/p%d.txt", world_size, my_rank);
    outfile = fopen(my_str, "a");
    fprintf(outfile, "MPI_Alltoall-1: time: %lf \n", t_latency);
    fclose(outfile);
    ...
    snprintf(my_str, sizeof(my_str), "/scratch/07431/tg867305/process_skew/miniamr/%d/p%d.txt", world_size, my_rank);
    outfile = fopen(my_str, "a");
    fprintf(outfile, "MPI_Alltoall-2: time: %lf \n", t_latency);
    fclose(outfile);
    t_start = MPI_Wtime(); //added
    ...
    snprintf(my_str, sizeof(my_str), "/scratch/07431/tg867305/process_skew/miniamr/%d/p%d.txt", world_size, my_rank);
    outfile = fopen(my_str, "a");
    fprintf(outfile, "MPI_Allreduce-10: time: %lf \n", t_latency);
    fclose(outfile);
         if (!my_pe && report_perf & 8)
...
void sort(int div, int fact, int dir)
{
   ...
   MPI_Allreduce(&num_dots, &total_dots, 1, MPI_INT, MPI_SUM, comms[div]);
    ...
    snprintf(my_str, sizeof(my_str), "/scratch/07431/tg867305/process_skew/miniamr/%d/p%d.txt", world_size, my_rank);
    outfile = fopen(my_str, "a");
    fprintf(outfile, "MPI_Allreduce-11: time: %lf \n", t_latency);
    fclose(outfile);
   ...
   MPI_Allreduce(bin, gbin, mesh_size[dir], MPI_INT, MPI_SUM, comms[div]);
    ...
    snprintf(my_str, sizeof(my_str), "/scratch/07431/tg867305/process_skew/miniamr/%d/p%d.txt", world_size, my_rank);
    outfile = fopen(my_str, "a");
    fprintf(outfile, "MPI_Allreduce-12: time: %lf \n", t_latency);
    fclose(outfile);
   ...
   if (group_blocks) {
    ...
    snprintf(my_str, sizeof(my_str), "/scratch/07431/tg867305/process_skew/miniamr/%d/p%d.txt", world_size, my_rank);
    outfile = fopen(my_str, "a");
    fprintf(outfile, "MPI_Allreduce-13: time: %lf \n", t_latency);
    fclose(outfile);
            part = bin1[j] - extra[j];
    ...
    snprintf(my_str, sizeof(my_str), "/scratch/07431/tg867305/process_skew/miniamr/%d/p%d.txt", world_size, my_rank);
    outfile = fopen(my_str, "a");
    fprintf(outfile, "MPI_Allreduce-14: time: %lf \n", t_latency);
    fclose(outfile);
               part = bin1[j] - extra1;
               for (sum = i = 0; i < mesh_size[dir1]; i++) {
                  sum += gbin[i];
...
void move_blocks(double *tp, double *tm, double *tu)
{
   ...
   do {
    ...
    snprintf(my_str, sizeof(my_str), "/scratch/07431/tg867305/process_skew/miniamr/%d/p%d.txt", world_size, my_rank);
    outfile = fopen(my_str, "a");
    fprintf(outfile, "MPI_Allreduce-15: time: %lf \n", t_latency);
    fclose(outfile);
   } while (n && k != n);
