...
int main(int argc, char* argv[]) {
  ...
  gettimeofday(&start, NULL);
    ...
    double t_1, t_2, t_3, t_latency1, t_latency2;
    FILE* outfile;
    int my_rank, world_size;
    MPI_Comm_rank( MPI_COMM_WORLD, &my_rank );
    MPI_Comm_size(MPI_COMM_WORLD, &world_size);
    char my_str[128];
    snprintf(my_str, sizeof(my_str), "/scratch/07431/tg867305/process_skew/halo3d/%d/p%d.txt", world_size, my_rank);
  ...
  for (int i = 0; i < repeats; ++i) {
    ...
    t_3 = MPI_Wtime(); //added
    t_latency1 = (double)((t_2 - t_1) * 1e6);
    t_latency2 = (double)((t_3 - t_1) * 1e6);
    outfile = fopen(my_str, "a");
    fprintf(outfile, "SendRecv-1: time: %lf \n", t_latency1);
    fprintf(outfile, "SendRecvWait-1: time: %lf \n", t_latency2);
    fclose(outfile);
